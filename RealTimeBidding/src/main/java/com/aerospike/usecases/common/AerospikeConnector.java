...
/**
 * An abstraction class to parse command line options pertaining to connecting to the cluster.
 * <p/>
...
public class AerospikeConnector {
    ...
    public Options getOptions() {
        ...
        options.addOption("h", "hosts", true, 
                "List of seed hosts for cluster in format: " +
        ...
        options.addOption("t", "tls", true, "Set the TLS Policy options on the cluster. The value passed should be a JSON string. Valid keys in this "
        ...
        options.addOption("cl", "useCloud", false, "Use Aerospike cloud. If this option is set, TLS cannot be set");
        return options;
    }
    
    /**
     * Validate the passed options as they pertain to connecting to the cluster. If there are validation
    ...
    /**
     * Connect to the Aerospike cluster using the command line options. The passed <pre>ClientPolicy</pre>
     * will be used to connect with the connection details (hosts, username, etc) overridden by the command
     * line. This is useful if the various <pre>defaultXXXPolicy</pre> need to be overridden.
     * @param policy
     * @return A connection to the Aerospike database
     */
