...
public class DataPopulator {
    ...
    /**
     * Generate the devices and segments for a range of ids. Note that in a real
     * world scenario the ids would be systematically generated like UUIDs or
     * similar. However, for ease of simulating this process
     * 
     ...
     */
    private void generateDevices(TimingMetric timer, long numberOfSegments, long startDevice, long endDevice,
            long avgSegmentsPerDevice) {
        if (Log.debugEnabled()) {
            Log.debug(String.format("    generateDevices(%d, %d, %d, %d)\n", numberOfSegments, startDevice, endDevice,
        ...
        for (long thisDeviceId = startDevice; thisDeviceId < endDevice; thisDeviceId++) {
            ...
            long segmentsToGenerate = Math.max(0,
                    Math.min(1000, (long) (avgSegmentsPerDevice + (avgSegmentsPerDevice / 2) * random.nextGaussian())));
    ...
    public void generateDevices(long numberOfSegments, long numberOfDevices, long avgSegmentsPerDevice,
            int numberOfThreads) {
        if (Log.debugEnabled()) {
            Log.debug(String.format("generateDevices(%d, %d, %d, %d)\n", numberOfSegments, numberOfDevices,
                    avgSegmentsPerDevice, numberOfThreads));
        }
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            executor.submit(() -> {
                this.generateDevices(timer, numberOfSegments, startDevice, startDevice + thisDevices,
                        avgSegmentsPerDevice);
            });
        }
        executor.shutdown();
