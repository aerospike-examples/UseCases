...
public class RealTimeBidding {
    ...
    public static void main(String[] args) throws Exception {
        ...
        options.addOption("d", "device", true,
                "Specify the device id (number) to use in request. The device id will be turned into a true string id.");
        ...
        options.addOption("nS", "numSegments", true,
                "The number of segment to use when generating. This just constrains the randomly selected segments to this range.");
        options.addOption("aS", "avgSegmentsPerDevice", true,
                "The average number of segments per device for generation. Defaults to 100");
        options.addOption("nt", "numThreads", true,
                "The number of threads to use when generating data. Defaults to one thread per CPU core");
        ...
        case "generate":
            ...
            try (IAerospikeClient client = connector.connect()) {
                ...
                populator.generateDevices(Long.parseLong(cl.getOptionValue("numSegments")),
                        Long.parseLong(cl.getOptionValue("numDevices")),
                        Long.parseLong(cl.getOptionValue("avgSegmentsPerDevice", "100")), numThreads);
            }
            break;
